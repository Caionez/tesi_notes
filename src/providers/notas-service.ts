...
export class NotasService {
  ...
  getNotas(nomeStorage: string) {
    return this.storage.get(nomeStorage).then(result => {
  ...
  setNotasStorage(nomeStorage: string, callback?) {
    this.storage
      .set(nomeStorage, JSON.stringify(this.notas))
      .then(
        result => (callback ? callback(result) : console.log(result)),
  ...
  moverNota(
    codigo: number,
    nomeStorageOrigem: string,
    nomeStorageDestino: string
  ) {
    let notaMovida = this.getNota(codigo);
    let novoCodigo: number = 0;

    this.storage.get(nomeStorageDestino).then(result => {
      let notasDestino: any[] = [];

      if (result != undefined) notasDestino = JSON.parse(result);
      ...
      notasDestino.push({
        codigo: ++novoCodigo,
        titulo: notaMovida.titulo,
        texto: notaMovida.texto,
        cor: notaMovida.cor
      });
